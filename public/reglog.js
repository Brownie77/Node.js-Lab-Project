...
async function loginUser() {
  document
    .getElementById('login-user-form')
    .addEventListener('click', function (event) {
      event.preventDefault();
    });
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      window.location.replace(
        `http://localhost:3000/static/patient.html?${user.id}`,
      );
    }
  } catch (error) {}
}
...
async function loginDoctor() {
  document
    .getElementById('doctor-login-user-form')
    .addEventListener('click', function (event) {
      event.preventDefault();
    });
  const formData = new FormData(
    document.getElementById('doctor-login-user-form'),
  );
  const dataTransfer = {};
  for (let pair of formData.entries()) {
    Object.defineProperty(dataTransfer, pair[0].toString(), {
  ...
  try {
    ...
    if (response.status === 200) {
      window.location.href = 'http://localhost:3000/static/doctor.html';
    }
  } catch (error) {}
}
